...
import com.knesarcreation.playbeat.activity.equailizer.AnalogController.onProgressChangedListener
import com.knesarcreation.playbeat.fragment.AllSongFragment
import com.knesarcreation.playbeat.utils.EqualizerModel
...
//
        if (AllSongFragment.musicService != null) {
            try {
                AllSongFragment.musicService!!.mEqualizer = Equalizer(1000, audioSesionId)
                AllSongFragment.musicService!!.bassBoost = BassBoost(1000, audioSesionId)
                AllSongFragment.musicService!!.presetReverb = PresetReverb(1000, audioSesionId)
...
//        }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        equalizerSwitch!!.setOnCheckedChangeListener { _, isChecked ->
            if (AllSongFragment.musicService != null) {
                AllSongFragment.musicService!!.mEqualizer!!.enabled = isChecked
                AllSongFragment.musicService!!.bassBoost!!.enabled = isChecked
                AllSongFragment.musicService!!.presetReverb!!.enabled = isChecked
        ...
        if (AllSongFragment.musicService != null) {
            if (!Settings.isEqualizerReloaded) {
                ...
                if (AllSongFragment.musicService!!.bassBoost != null) {
                    try {
                        x = AllSongFragment.musicService!!.bassBoost!!.roundedStrength * 19 / 1000
                    } catch (e: Exception) {
                        e.printStackTrace()
                    }
                }
                if (AllSongFragment.musicService!!.presetReverb != null) {
                    try {
                        y = AllSongFragment.musicService!!.presetReverb!!.preset * 19 / 6
                    } catch (e: Exception) {
                        e.printStackTrace()
                    }
                }
        ...
        val lowerEqualizerBandLevel = AllSongFragment.musicService!!.mEqualizer!!.bandLevelRange[0]
        val upperEqualizerBandLevel = AllSongFragment.musicService!!.mEqualizer!!.bandLevelRange[1]
        ...
        for (i in 0 until numberOfFrequencyBands) {
            ...
            val centralFreq =
                AllSongFragment.musicService!!.mEqualizer!!.getCenterFreq(equalizerBandIndex) / 1000
            if ((centralFreq) >= 1000) {
                frequencyHeaderTextView.text =
                    "%.0f".format(((centralFreq).toDouble() / 1000)) + "kHz"
            } else
                frequencyHeaderTextView.text =
                    (AllSongFragment.musicService!!.mEqualizer!!.getCenterFreq(equalizerBandIndex) / 1000).toString() + "Hz"
            ...
            if (Settings.isEqualizerReloaded) {
                ...
                // dataset!!.addPoint(frequencyHeaderTextView.text.toString(), points[i])
                seekBar.progress =
                    AllSongFragment.musicService!!.mEqualizer!!.getBandLevel(equalizerBandIndex) - lowerEqualizerBandLevel
    ...
    private fun updateUI(isChecked: Boolean) {
        ...
        for (i in 0 until AllSongFragment.musicService!!.mEqualizer!!.numberOfBands) {
        ...
        if (!isChecked) {
            ...
            bassController!!.circlePaint2!!.color =
                ContextCompat.getColor(ctx!!, R.color.app_theme_default_light)
            ...
            bassController!!.circlePaint2!!.color =
                ContextCompat.getColor(ctx!!, R.color.teal_200)
            bassController!!.linePaint!!.color =
                ContextCompat.getColor(ctx!!, R.color.teal_200)
            reverbController!!.circlePaint2!!.color =
                ContextCompat.getColor(ctx!!, R.color.teal_200)
    ...
    private fun equalizeSound() {
        ...
        for (i in 0 until AllSongFragment.musicService!!.mEqualizer!!.numberOfPresets) {
            equalizerPresetNames.add(AllSongFragment.musicService!!.mEqualizer!!.getPresetName(i.toShort()))
        }
            ...
            AdapterView.OnItemClickListener { p0, p1, position, p2 ->
                ...
                try {
                    if (position != 0) {
                        ...
                        AllSongFragment.musicService!!.mEqualizer!!.usePreset((position - 1).toShort())
                        Settings.presetPos = position
                        val lowerEqualizerBandLevel =
                            AllSongFragment.musicService!!.mEqualizer!!.bandLevelRange[0]

                        for (i in 0 until numberOfFreqBands) {
                            seekBarFinal[i]!!.progress =
                                AllSongFragment.musicService!!.mEqualizer!!.getBandLevel(i.toShort()) - lowerEqualizerBandLevel
                            //points[i] =
