...
import com.knesarcreation.playbeat.util.Utility
...
class AudioTrimmerActivity : AppCompatActivity(), View.OnClickListener, MarkerView.MarkerListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent != null) {
            ...
            binding.audioName.text = allSongModle.title
        }
    ...
    override fun onClick(view: View) {
        if (view === binding.txtAudioPlay) {
            if (!mIsPlaying) {
                ...
                binding.txtAudioPlay.setImageResource(R.drawable.ic_play_arrow)
            }
            onPlay(mStartPos)
        } else if (view === binding.trimAudio) {
            val alertSaveTrimmedAudio = MaterialAlertDialogBuilder(this)
            ...
            etAudioName.setText("PlayBeat_${allSongModle.title}")
...
//                    handlePause();
    ...
    private fun afterSavingAudioFile(
        ...
        values.put(MediaStore.Audio.Media.ARTIST, allSongModle.artistName)
            ...
            override fun run() {
                try {
                    ...
                    val runnable = Runnable { //mPlayer = new SamplePlayer(mLoadedSoundFile);
                        ...
                        binding.audioWaveform.setBackgroundColor(resources.getColor(R.color.black_color))
