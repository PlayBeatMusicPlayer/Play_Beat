    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        when (getItemViewType(position)) {
            ALBUM -> {
                ...
                GlideApp.with(activity).asDrawable().albumCoverOptions(album.safeGetFirstSong())
                    .load(
                        PlayBeatGlideExtension.getSongModel(album.safeGetFirstSong())
                    )
                    .into(holder.image!!)
            }
            ARTIST -> {
                ...
                GlideApp.with(activity).asDrawable().artistImageOptions(artist).load(
                    PlayBeatGlideExtension.getArtistModel(artist)
                ).into(holder.image!!)
            }
            SONG -> {
                holder.imageTextContainer?.isVisible = true
                ...
                GlideApp.with(activity).asDrawable().songCoverOptions(song)
                    .load(PlayBeatGlideExtension.getSongModel(song)).into(holder.image!!)
            }
            GENRE -> {
        ...
        init {
            ...
            if (itemViewType == SONG) {
                ...
                menu?.setOnClickListener {
                    val song = dataSet[layoutPosition] as Song
